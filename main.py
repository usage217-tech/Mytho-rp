...
WEBAPP_URL = "https://usage217-tech.github.io/Mytho-rp/" # <--- REPLACE THIS AFTER GITHUB UPLOAD
...
async def handle_manifest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        char_desc = data.get('charDesc', 'A mysterious figure')
        user_name = data.get('userName', 'Traveler')
        scenario = data.get('scenario', 'A meeting in the dark.')

        # --- EXACT SYSTEM PROMPT FROM YOUR FILE ---
        system_prompt = (
            f"You are playing as {char_name}.\n"
            f"Personality: {char_desc}. User: {user_name}.\n"
